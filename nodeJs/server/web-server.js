...
var util = require('util'),
    ...
    program = require('commander');   //获取命令行参数模板吧
ejs.ROOT_PATH = __dirname+"/../templates";
var DEFAULT_PORT = 8877;
var env = "DEVELOPMENT";
var envLev = 0;//运行环境
...
(function(){
      ...
      try{
        res.write(ejs.render(templates,{
          ...
          _build:{
            pkg:pkg,
            version: "REGULP v1.0",
            ts: parseInt( req.url.query.debug ) ? "" : new Date().getTime(),
            doMinify: envLev > 1,
            env: 'DEVELOPMENT'
          },
          ctx:""
  ...
  ChildServlet.prototype.handleRequest = function(req,res){
    ...
    if(node_path.extname(path) == ".html" && !fs.existsSync(path)){
      ...
      //这个就是执行   StaticServlet.handleRequest。。。。即直接输出了 文件内容。
    }else{
...
program
    ...
    .option("-p, --port [port]", "设置运行端口", /^[0-9]{4,5}$/, DEFAULT_PORT)
    .parse(process.argv);
main(program);
