...
function SignatureConfig(signer, signRule)
{
   ...
   // this.title = "本人已阅读保险条款、产品说明书和投保提示书，了解本产品的特点和保单利益的不确定性。";//批示输入有效，批示内容
    this.cid = 0;
		var signerUnameLen=this.signer.UName.length;
    var titleToOffset=4+signerUnameLen-1;
    this.title = "请授权人"+this.signer.UName+"签名";//签字输入有效，签字框标题
    this.titleSpanFromOffset = 4;//当为普通签名时有效，表示title中需要放大显示字体的数组起始index
    this.titleSpanToOffset = titleToOffset;//当为普通签名时有效，表示title中需要放大显示字体的数组结束index
