...
//这样可以区分 Symbol是系统函数, 还是用户自定义了一个Symbol, 下面这个函数可以看出来
     ...
     */
     // @Jane 创建 Observe,并将value 加入watcher监听？
     ...
     * * Jane 如果有 childVal，要将parentVal，一起合并起来。否则返回parentVal
     */
    function mergeHook(
        parentVal,
        childVal
    ...
    Watcher.prototype.get = function get() {
        ...
        if (this.user) {
            ...
            //@Jane  getter一般是options.computed,属性值，即函数。
            value = this.getter.call(vm, vm);
        }
    ...
    function initData(vm) {
        ...
        while (i--) {
            ...
            // @Jane isReserved :(检查是否 以$或_开头的字符串？) Check if a string starts with $ or _
            } else if (!isReserved(keys[i])) {
            //@Jane 设置vm的setter,getter暂时为空  vm.keys[i]即是在vm._data里取
    ...
    // Jane 默认情况下，一个组件的 v-model 会使用 value 属性和 input 事件，但是诸如单选框、复选框之类的输入类型可能把 value 属性用作了别的目的。model 选项可以回避这样的冲突：
    // model: {
    //           prop: 'checked',
    //           event: 'change'
    //       },
    ...
    initMixin(Vue$3);
    //Jane 设置vue原型状态：$data（getter/setter），$props，$watch
    ...
    function initExtend(Vue) {
        ...
        Vue.extend = function(extendOptions) {
            ...
            if (Sub.options.props) {
                //Sub的props，里的取值设置setter,getter.是从this._prop里取跟设置。
                initProps$1(Sub);
            }
    ...
    function initAssetRegisters(Vue) {
         ...
         *  Jane,注册 组件函数，  ASSET_TYPES ＝ 有 component...  
            var ASSET_TYPES = [
                'component',
                'directive',
                'filter'
            ];
         *  缓存在 this.options[ASSET_TYPES+s]里。
         */
        ASSET_TYPES.forEach(function(type) {
            Vue[type] = function(
                id, //这个可以是 tag 元素
                ...
                if (!definition) {
                    ...
                    if (type === 'component' && isPlainObject(definition)) {
                        ...
                        //复制遗传一份vue的功能给到,component的。  this.options._base == Vue;
    ...
    function initGlobalAPI(Vue) {
        ...
        // 初始化 vue 类的继承
        initExtend(Vue);
        // 初始vue.component, directive, filter 这些函数。
         // ASSET_TYPES = [
            //     'component',
            //     'directive',
            //     'filter'
...
// MathML 是数学标记语言，是一种基于XML（标准通用标记语言的子集）的标准，用来在互联网上书写数学符号和公式的置标语言。
    ...
    function createPatchFunction(backend) {
        ...
        function createElm(vnode, insertedVnodeQueue, parentElm, refElm, nested) {
            ...
            if (isDef(tag)) {
                ...
                /* istanbul ignore if */
                {
                    ...
                    //处理节点中的数据， directives,domprop,event
                    if (isDef(data)) {
                    // var hooks = ['create', 'activate', 'update', 'remove', 'destroy'];
        ...
        function createComponent(vnode, insertedVnodeQueue, parentElm, refElm) {
            var i = vnode.data;
            if (isDef(i)) {
                //todo...@componentInstance子节点的虚拟实例。 和 @keepAlive 分别代表什么？
    ...
    var model$1 = {
        // 元素data里的hook的 insert 触发勾子  todo。。。。
    ...
    function decode(html) {
        ...
        //Jane textContent 属性设置或返回指定节点的文本内容，以及它的所有后代。
    ...
    function parseHTML(html, options) {
        ...
        while (html) {
            ...
            if (!lastTag || !isPlainTextElement(lastTag)) {
                ...
                if (textEnd === 0) {
                    ...
                    // Start tag:  @Jane 获取到开始标签元素，handleStartTag处理 开始标签的 自定义命令。
        ...
        function parseStartTag() {
            ...
            if (start) {
                ...
                if (end) {
                    //unarySlash 为空则是正常标签，即需要有闭合元素标签。 否则不需要。
    ...
    var bindRE = /^:|^v-bind:/;
    var modifierRE = /\.[^.]+/g;
    // @decode  func: 设置或返回指定节点的文本内容，以及它的所有后代。
    // cached: func: 单例函数，缓存结果。
