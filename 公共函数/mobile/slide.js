iuni.mobile.plug.hori_slider.slip=(function(window, document) {
    _fun = {
        ios: function() {
            // 作用：判断是否为苹果的IOS设备
    ...
    var slipjs = {
        ...
        __posX: function(x) {
            // 作用：当设置滑动的方向为"X"时用于设置滑动元素的坐标
            this.xy = x;
            this.core.style['webkitTransform'] = 'translate3d(' + x + 'px, 0px, 0px)';
            //this.core.style['webkitTransform'] = 'translate('+x+'px, 0px) scale(1)';

        },
        __posY: function(x) {
            // 作用：当设置滑动的方向为"Y"时用于设置滑动元素的坐标
            this.xy = x;
            this.core.style['webkitTransform'] = 'translate3d(0px, ' + x + 'px, 0px)';
            //this.core.style['webkitTransform'] = 'translate(0px, '+x+'px) scale(1)';
    ...
    SlipPage._end = function(e) {
        ...
        if (!this._movestart) {
            ...
            } else if (Math.abs(that._range) > 80) {
    ...
    SlipPage.toPage = function(num, time) {
        // 可在外部调用的函数，指定轮换到第几张，只要传入："轮换到第几张"和"时间"两个参数。
    ...
    SlipPage.refresh = function(wide_high, parent_wide_high) {
        // 可在外部调用，作用：当尺寸改变时（如手机横竖屏时），需要重新取得相关的值。这时候就可以调用该函数
    ...
    SlipPx.__posBarX = function(pos) {
        // 作用：当设置滑动的方向为"X"时用于设置滚动条的坐标 
        var that = this;
        that._bar.style['webkitTransform'] = 'translate3d(' + pos + 'px, 0px, 0px)';
        //that._bar.style['webkitTransform'] = 'translate('+pos+'px, 0px)';

    };
    SlipPx.__posBarY = function(pos) {
        // 作用：当设置滑动的方向为"Y"时用于设置滚动条的坐标 
        var that = this;
        //that._bar.style['webkitTransform'] = 'translate(0px, '+pos+'px)';
    ...
    SlipPx.refresh = function(wide_high, parent_wide_high) {
        // 可在外部调用，作用：当尺寸改变时（如手机横竖屏时），需要重新取得相关的值。这时候就可以调用该函数
        var that = this;
        slipjs._refreshCommon(wide_high, parent_wide_high);
        if (!that.no_bar) {
            if (that.coreWidth_cut_width <= 0) {
                that._bar_shell_opacity = 0;
                that._showBar = function() {};
